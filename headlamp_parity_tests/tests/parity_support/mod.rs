...
fn ensure_pytest_venv_bin_dir() -> Option<PathBuf> {
    ...
    PY_BIN
        .get_or_init(|| {
            ...
            // Prefer the repo's checked-in venv (fast, no network).
            let repo_venv = requirements
                .parent()
                .unwrap()
                .join(".venv");
            ...
            // Fall back to creating a venv under this crate's target directory.
...
fn run_cmd_tty(mut cmd: Command, columns: usize) -> (i32, String) {
    let _timing = timing::TimingGuard::start("tty_run");
    ...
    let (code, stderr_text) = match status {
        ...
        None => {
            ...
            (
                124,
                format!(
                    "[headlamp_parity_tests] timeout after {}s (killed)\n",
                    timeout.as_secs()
                ),
            )
        }
...
fn run_cmd_tty_stdout_piped(mut cmd: Command, columns: usize) -> (i32, String) {
    let _timing = timing::TimingGuard::start("tty_stdout_piped_run");
    cmd.env("TERM", "xterm-256color");
    cmd.env("CI", "1");
    cmd.env_remove("NO_COLOR");
...
fn run_cmd_tty_portable_pty(
    ...
    let mut status = None;
    while status.is_none() && start.elapsed() < timeout {
    ...
    drop(pair.master);
    let bytes = read_thread.join().unwrap_or_default();
    let mut combined = String::from_utf8_lossy(&bytes).to_string();
    combined = combined
        .replace(['\u{0008}', '\u{0004}'], "")
        .replace("^D", "");
...
pub fn run_parity_headlamp_vs_headlamp_with_args_tty(
    ...
    let baseline_spec = mk_headlamp_tty_run_spec(
        repo,
        headlamp_bin,
        columns,
        baseline_runner,
