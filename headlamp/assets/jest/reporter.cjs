...
const sanitizeError = (err) => {
  ...
  const out = {};
  const name =
    err.name || (err.constructor && err.constructor.name) || undefined;
  ...
  if (err.matcherResult && isObject(err.matcherResult)) {
    ...
    out.matcherResult = {
      matcherName:
        typeof mr.matcherName === "string" ? mr.matcherName : undefined,
      message: messageText,
...
const sanitizeDetail = (d) => {
  ...
  if (d.matcherResult) {
    out.matcherResult = sanitizeError({
      matcherResult: d.matcherResult,
    }).matcherResult;
  }
...
class BridgeReporter {
  ...
  onTestResult(_test, tr) {
    ...
    this.buf.testResults.push({
      ...
      status:
        (tr &&
          typeof tr.numFailingTests === "number" &&
          tr.numFailingTests > 0) ||
      ...
      timedOut: Boolean(
        (tr.testExecError &&
          /timed out/i.test(
            String(
              tr.testExecError && (tr.testExecError.message || tr.testExecError)
            )
          )) ||
  ...
  onRunComplete(_contexts, agg) {
    const suiteTimedOut = (r) =>
      Boolean(
        (r.testExecError &&
          /timed out/i.test(
            String(
              r.testExecError && (r.testExecError.message || r.testExecError)
            )
          )) ||
    ...
    this.buf.aggregated = {
      ...
      runTimeMs: agg.testResults.reduce(
        (t, r) =>
          t + Math.max(0, (r.perfStats?.end || 0) - (r.perfStats?.start || 0)),
        0
      ),
    };
