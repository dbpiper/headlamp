  ...
  try {
    ...
    const getCtx = () => {
      try {
        ...
        const testPathCandidate =
          typeof st.testPath === "string"
            ? st.testPath
            : inferTestPathFromStack();
        const testPath =
          typeof testPathCandidate === "string" ? testPathCandidate : "";
        const currentTestName =
          typeof st.currentTestName === "string" ? st.currentTestName : "";
    ...
    try {
      ...
      for (const lvl of levels) {
        try {
          originals[lvl] =
            g.console[lvl] && g.console[lvl].bind
              ? g.console[lvl].bind(g.console)
              : g.console[lvl];
          g.console[lvl] = (...args) => {
            try {
