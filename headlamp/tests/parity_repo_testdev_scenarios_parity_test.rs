...
fn parity_repo_test_dev_works_for_untracked_changed_mode() {
    let Some(binaries) = parity_binaries() else { return };
    let Some(repo) = discover_repo() else { return };
    let Some(e2e) = setup_repo(repo, &binaries) else { return };
...
fn parity_repo_test_dev_failing_test_with_show_logs() {
    ...
    assert_parity(&e2e, "failing_show_logs", code_ts, &out_ts, code_rs, &out_rs);
}
