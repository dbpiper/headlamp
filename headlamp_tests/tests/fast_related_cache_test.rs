...
fn cached_related_uses_env_cache_root_and_hits_cache() {
    ...
    let first = cached_related(&repo_root, selection_key, false, compute_first).unwrap();
    ...
    let second = cached_related(&repo_root, selection_key, false, compute_second).unwrap();
    assert_eq!(call_count.load(Ordering::SeqCst), 1);
...
fn cached_related_recomputes_when_cached_paths_are_missing() {
    ...
    let _ = cached_related(&repo_root, selection_key, false, compute_first).unwrap();
    ...
    let _ = cached_related(&repo_root, selection_key, false, compute_second).unwrap();

    assert_eq!(call_count.load(Ordering::SeqCst), 2);
}

#[test]
fn cached_related_no_cache_bypasses_cache_and_recomputes() {
    let _lock = ENV_LOCK.lock().unwrap();
    let cache_root = unique_temp_dir("headlamp-tests-cache-nocache");
    let _ = std::fs::remove_dir_all(&cache_root);
    std::fs::create_dir_all(&cache_root).unwrap();
    let _env = EnvVarGuard::set("HEADLAMP_CACHE_DIR", &cache_root.to_string_lossy());

    let repo_root = unique_temp_dir("headlamp-tests-repo-nocache");
    let _ = std::fs::remove_dir_all(&repo_root);
    std::fs::create_dir_all(&repo_root).unwrap();
    ...
    let _ = cached_related(&repo_root, selection_key, true, compute_first).unwrap();
    let compute_second = {
        let call_count = Arc::clone(&call_count);
        let path_text = existing_test.to_string_lossy().to_string();
        move || {
    ...
    let _ = cached_related(&repo_root, selection_key, true, compute_second).unwrap();
